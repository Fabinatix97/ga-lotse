...
@Service
public class StatisticCopyService {
  ...
  public UUID addCopy(CloneStatisticRequest cloneStatisticRequest) {
    ...
    copy.addEvaluations(copyEvaluations(original.getEvaluations()));

    return statisticRepository.save(copy).getExternalId();
  }
  ...
  private List<Evaluation> copyEvaluations(List<Evaluation> evaluations) {
  ...
  private static BarChartConfiguration copyBarChartConfiguration(BarChartConfiguration original) {
    ...
    Optional.ofNullable(original.getSecondaryAttributeSelection())
        .map(StatisticCopyService::copyAttributeSelection)
  ...
  void copyTableRows(Statistic copy, Statistic original) {
    ...
    Page<TableRow> tableRows =
        tableRowRepository.findAllByAggregationResult(
            original, PageRequest.of(page, tableRowPageSize));
