 ...
 */
public class Gdt35RecordBuilder extends BaseBuilder<Gdt35RecordBuilder> {

  private final String recordType;

  private Gdt35RecordBuilder(String recordType) {
   ...
   */
  public static Gdt35RecordBuilder requestMasterData() {
    return create("6300");
  }
   ...
   */
  public static Gdt35RecordBuilder transmitMasterData() {
    return create("6301");
  }
   ...
   */
  public static Gdt35RecordBuilder requestNewExamination() {
    return create("6302");
  }
   ...
   */
  public static Gdt35RecordBuilder cancelExamination() {
    return create("6303");
  }
   ...
   */
  public static Gdt35RecordBuilder transmitExaminationData() {
    return create("6310");
  }
   ...
   */
  public static Gdt35RecordBuilder showExaminationData() {
    return create("6311");
  }

  @Override
  protected Gdt35RecordBuilder self() {
    return this;
  }

  /**
   * Finalizes the construction and returns the immutable {@link Gdt35Record}.
   *
   * @return The constructed GDT record.
   */
  public Gdt35Record build() {
    validateMandatoryFields(recordType, elements);
    return new Gdt35Record(recordType, elements);
  }

  private void validateMandatoryFields(String recordType, List<Gdt35Element> elements) {
  ...
  private void requireExaminationResult(List<Gdt35Element> elements) {
    requireObject(elements, "Obj_0057", "Examination Result");
  }

  private void requireHeader(List<Gdt35Element> elements) {
    requireObject(elements, "Obj_0033", "Header");
  }

  private void requireExaminationRequest(List<Gdt35Element> elements) {
    requireObject(elements, "Obj_0012", "Examination Request");
  }

  private void requirePatient(List<Gdt35Element> elements) {
    requireObject(elements, "Obj_0045", "Patient");
  }

  private void requireRequestUid(List<Gdt35Element> elements) {
   ...
   */
  private void requireAttachmentIfPresent(List<Gdt35Element> elements) {
  ...
  private void requireObjectIfAttributePresent(
      List<Gdt35Element> elements, String attributeTag, String objectId, String name) {
    boolean attributePresent = elements.stream().anyMatch(e -> attributeTag.equals(e.getTag()));
    if (attributePresent) {
      elements.stream()
          .filter(Gdt35Object.isInstance())
          .map(Gdt35Object.cast())
  ...
  private void requireObject(List<Gdt35Element> elements, String objectId, String name) {
    findObject(elements, objectId, name);
  }

  private Gdt35Object findObject(List<Gdt35Element> elements, String objectId, String name) {
  ...
  private void requireFieldInObject(
      List<Gdt35Element> elements,
      String objectId,
      String objectName,
      String tag,
      String fieldName) {

    Gdt35Object targetObject = findObject(elements, objectId, objectName);

    boolean fieldExists =
        targetObject.elements().stream()
            .filter(Gdt35Field.isInstance())
            .map(Gdt35Field.cast())
   ...
   * encoding info.
   *
   * @param config A configuration consumer for the {@link Gdt35HeaderBuilder}.
   * @return This builder instance.
   */
  public Gdt35RecordBuilder header(Consumer<Gdt35HeaderBuilder> config) {
    return addObject("8133", "Kopfdaten_GDT", "Obj_0033", new Gdt35HeaderBuilder(), config);
  }
   ...
   *
   * @param config A configuration consumer for the {@link Gdt35PatientBuilder}.
   * @return This builder instance.
   */
  public Gdt35RecordBuilder patient(Consumer<Gdt35PatientBuilder> config) {
    return addObject("8145", "Patient", "Obj_0045", new Gdt35PatientBuilder(), config);
  }
   ...
   *
   * @param config A configuration consumer for the {@link Gdt35ExaminationRequestBuilder}.
   * @return This builder instance.
   */
  public Gdt35RecordBuilder examinationRequest(Consumer<Gdt35ExaminationRequestBuilder> config) {
    return addObject(
        "8112", "Anforderung", "Obj_0012", new Gdt35ExaminationRequestBuilder(), config);
  }
   ...
   *
   * @param config A configuration consumer for the {@link Gdt35ExaminationResultBuilder}.
   * @return This builder instance.
   */
  public Gdt35RecordBuilder examinationResult(Consumer<Gdt35ExaminationResultBuilder> config) {
    return addObject(
        "8157",
        "Untersuchungsergebnis_GDT",
        "Obj_0057",
        new Gdt35ExaminationResultBuilder(),
        config);
  }
   ...
   * @return This builder instance.
   */
  public Gdt35RecordBuilder attachment(Consumer<AttachmentBuilder> config) {
