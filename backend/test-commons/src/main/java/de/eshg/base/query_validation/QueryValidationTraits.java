...
public interface QueryValidationTraits extends JUnit5ValidationFileAssertions {
  ...
  default <T> T captureQueryAndCompareWithFile(
    ...
    return captureQueryAndCompareUsingAssertionFunction(
        callable,
        capturedQueries -> assertWithFileWithSuffix(capturedQueries, normalizer, SUFFIX_SQL));
  }
  ...
  default <T> T captureQueryAndCompareWithFile(
    ...
    return captureQueryAndCompareUsingAssertionFunction(
        callable, capturedQueries -> assertWithFileWithSuffix(capturedQueries, normalizer, suffix));
  }

  private <T> T captureQueryAndCompareUsingAssertionFunction(
      Callable<T> callable, Consumer<String> assertionFunction) {
    QueryAndParamsCapturingListener queryListener = getQueryListener();
    queryListener.startListening();
    try {
      T result = callable.call();
      String capturedQueries =
          queryListener.getCapturedQueries().stream()
              .map(QueryValidationTraits::createLogEntry)
  ...
  static String createLogEntry(CapturedQuery capturedQuery) {
    return new CustomQueryLogEntryCreator()
        .getLogEntry(capturedQuery.execInfo(), capturedQuery.queryInfoList(), true, false, true);
  }
