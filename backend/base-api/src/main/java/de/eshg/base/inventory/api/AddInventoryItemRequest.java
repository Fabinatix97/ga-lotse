...
public record AddInventoryItemRequest(
    ...
    @Schema(
            description =
                "The initial amount of stock of the Inventory Item, which shall be available for bookings.",
            example = "500")
        @NotNull
        @Min(0)
        int count, // TODO (ISSUE-1633): Remove this field, requires breaking change
    @Schema(
            description =
  ...
  public AddInventoryItemRequest(@NotBlank String name, @NotNull InventoryItemTypeDto type) {
    this(name, type, null, null, null, 0, 0);
  }

  public AddInventoryItemRequest(
      @NotBlank String name, InventoryItemTypeDto type, int count, int minCount) {
    this(name, type, null, null, null, count, minCount);
  }
}
