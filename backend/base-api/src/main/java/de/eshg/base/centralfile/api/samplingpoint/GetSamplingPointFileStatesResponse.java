...
public record GetSamplingPointFileStatesResponse(
    @ArraySchema(
            arraySchema = @Schema(description = "A list of requested File States of Facilities."))
        @Valid
        @NotNull
        List<GetSamplingPointFileStateResponse> samplingPointFileStates,
    @ArraySchema(
            arraySchema =
                @Schema(
                    description =
                        "A list containing the IDs of those requested facility file states which could not be found in the database.",
                    example =
