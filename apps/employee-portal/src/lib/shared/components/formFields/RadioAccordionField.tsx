...
const RadioAccordionGroupNameContext = createContext<string | undefined>(
  undefined,
);
...
export function RadioAccordionGroupField({
  ...
  return (
    <RadioGroupField {...props}>
      <RadioAccordionGroupNameContext value={props.name}>
        <Stack gap={2}>{children}</Stack>
      </RadioAccordionGroupNameContext>
    </RadioGroupField>
  );
}
...
export function RadioAccordionItem({
  ...
  const groupName = useContext(RadioAccordionGroupNameContext);

  if (groupName === undefined) {
    throw new Error("Missing RadioAccordionGroupNameContext");
  }
