...
import { useChatActivityTimeout } from "@/lib/businessModules/chat/shared/hooks/useChatActivityTimeout";
...
interface ChatClientContextType {
  ...
  refreshChatActivity: () => Promise<void>;
  chatActivityCountdown: number | null;
}
...
export function ChatClientProvider({ children }: Readonly<RequiresChildren>) {
   ...
   * Otherwise, a spinner or the passphrase view will be displayed.
   * The "Offline" state is an exception, allowing the rendering of an alert on the chat page.
   */
  const isClientPrepared =
    clientState === ClientState.Ready || clientState === ClientState.Idle;

  const { monitorChatActivity, refreshChatActivity, countdown } =
    useChatActivityTimeout(matrixClient, setClientState, clientState);
  ...
  const contextValues = useMemo<ChatClientContextType>(
    () => ({
      ...
      setCurrentSessionView,
      refreshChatActivity,
      chatActivityCountdown: countdown,
    }),
    [
      clientState,
      currentSessionView,
