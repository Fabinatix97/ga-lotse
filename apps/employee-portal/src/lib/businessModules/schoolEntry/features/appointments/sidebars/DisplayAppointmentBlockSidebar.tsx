...
export function DisplayAppointmentBlockSidebar(
  props: DisplayAppointmentBlockProps,
) {
  const { appointmentBlock, onClose } = props;
      ...
      <SidebarContent title="Terminblock">
        <Stack gap={2}>
          ...
          <Stack gap={2} role="list" aria-label="Termindetails">
            ...
            {appointmentBlock.creatorId && (
              <DetailsBlock title="Ersteller/Erstellerin:">
                <Typography>
                  {formatUserName(
                    appointmentBlock.resolvedUsers[appointmentBlock.creatorId],
                  )}
                </Typography>
              </DetailsBlock>
            )}
