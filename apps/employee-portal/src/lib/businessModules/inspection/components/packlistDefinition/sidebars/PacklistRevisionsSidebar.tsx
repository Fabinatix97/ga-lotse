...
import { DrawerProps, SidebarContent } from "@eshg/lib-employee-portal";
...
interface CreatePacklistRevisionsSidebarProps extends DrawerProps {
...
export function PacklistRevisionsSidebarWithQuery({
      ...
      <Stack direction="column-reverse">
        {revisions.map((revision, index) => (
          <PacklistDefinitionRevisionTile
            key={revision.id}
            revision={revision}
            previousName={index === 0 ? undefined : revisions[index - 1]?.name}
            version={version}
            label="versionTile"
