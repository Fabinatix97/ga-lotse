...
export function useQueryParamStateProvider(
  filterDefinitions: FilterDefinition[],
  {
    useRouterReplace = false,
    openParamName = "filtersOpen",
    openParamValue = "true",
  }: {
    openParamValue?: string;
    openParamName?: string;
    useRouterReplace?: boolean;
  } = {},
): FilterSettingsStateProvider {
  const [filterSettingsVisibleRaw, setFilterSettingsVisible] = useSearchParam(
    openParamName,
    "string",
  );
  const filterSettingsVisible = filterSettingsVisibleRaw === openParamValue;
  ...
  const setActiveValues = useCallback(
    (newActiveValues: FilterValue[]) => {
      const localSearchParams = windowSearchParams();
    ...
    [activeValues, useRouterReplace, router, pathname],
  );
  ...
  return {
    ...
    setFilterSettingsVisible: (a: SetStateAction<boolean>) => {
      ...
      setFilterSettingsVisible(next ? openParamValue : null);
    },
    activeValues,
    setActiveValues,
    errorMessages,
