...
export interface VaccineFormLoadings {
  ...
  inventoryVaccines: ApiInventoryVaccineWithoutRmbiVaccine[] | undefined; // undefined while loading
}
...
interface VaccineFormProps {
  ...
  onSubmit: (values: VaccineFormData) => Promise<void>;
  onCancel: () => void;
  title: () => string;
...
function validateForm(values: VaccineFormData) {
  const errors = {
    offsets: [] as string[],
  };
...
function inventoryVaccinesSelect(loadings: Readonly<VaccineFormLoadings>) {
  ...
  if (loadings.inventoryVaccines === undefined) {
    textHint = "Zuweisbare Inventar-Impfstoffe werden geladen...";
  } else if (loadings.inventoryVaccines.length == 0) {
  ...
  return (
    <SelectField
      ...
      disabled={loadings.inventoryVaccines === undefined}
      options={
        loadings.inventoryVaccines?.map(mapInventoryVaccinesToOption) ?? []
      }
      hint={textHint}
      required={textRequired}
    />
...
export function EditVaccineContent(props: Readonly<VaccineFormProps>) {
  return (
    <Formik
