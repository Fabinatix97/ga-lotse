...
import {
  ...
  ApiAppointmentType,
  ApiGetFreeAppointmentsResponse,
...
import { useGetFreeAppointments } from "@/lib/businessModules/travelMedicine/api/queries/appointmentBlocks";
...
export function AppointmentRadioGroup(
  ...
  const freeConsultationBlockAppointments = useGetFreeAppointments(
    ApiAppointmentType.Consultation,
  ).data;
  const freeVaccinationBlockAppointments = useGetFreeAppointments(
    ApiAppointmentType.Vaccination,
  ).data;

  function createAppointmentOptions(
    availableBlockAppointments: ApiGetFreeAppointmentsResponse,
  ) {
    let needToAddOption = true;
    const labelOptions: SelectOption[] =
      availableBlockAppointments.appointments.map((blockAppointment) => {
